...
CREATE VIEW bye_round_of_players AS SELECT id, name, count(matches.player1_id) FROM players LEFT JOIN (SELECT player1_id FROM matches WHERE player2_id is NULL) AS matches ON matches.player1_id = players.id GROUP BY players.id;

CREATE VIEW standing AS SELECT players.id, players.name, match2.wins, match1.matches from players
