...
def gconnect():
    ...
    # Check if the user is already logged in by checking if
    ...
    # use the credential access token to request logged in user info
    # from Google
    ...
    output += '!</h1>'
    output += '<img class="login-profile-pic" src="'
...
def showLogin():
    csrf_token = ''.join(random.choice(
        string.ascii_letters + string.digits) for x in xrange(32))
...
def home():
    ...
    timeDifference = currentTime - datetime.timedelta(
        days=NEW_ARTIST_EXPIRY_DURATION)
...
def showArtist(artist_id):
    ...
    return render_template('showArtist.html',
                           genres=genres,
                           artist=artist,
                           biography=biography)
...
def editArtist(artist_id):
    ...
    try:
        ...
        return redirect(url_for('home'))
    if is_user_authed(editedArtist.user_id, login_session['user_id']) \
...
def deleteArtist(artist_id):
    try:
        ...
        return redirect(url_for('home'))
    if is_user_authed(deletedArtist.user_id, login_session['user_id']) \
            and request.method == 'POST':
